...
def predict_mol(m, model, sclr,i, model_type, opt_noH, frags=None, per_atom_fragments=None, id_field_name=None):
    def pred(mol, model, opt_noH):
    ...
    if per_atom_fragments:
        ...
        for idx in range(m.GetNumAtoms()):
            if m.GetAtomWithIdx(idx).GetAtomicNum() > 1:
                ...
                mol_dict[nm + mol_frag_sep + str(idx + 1) + "#" + str(counter)] = pred(rw_m, model, opt_noH)
                counter += 1
        ...
        for k, v in frags[nm].items():
            ...
            mol_dict[nm + mol_frag_sep + k] = pred(rw_m, model, opt_noH)
    return mol_dict

def main_params(in_fname, out_fname, model_path, model_type, opt_noH, frag_fname,
                per_atom_fragments, id_field_name, multitask, threshold, save_pred=True, num_frag_id=False):
    # load model
    arguments = [
        '--test_path', '/dev/null',
        '--preds_path', '/dev/null',
    ...
    if input_file_extension == 'sdf':
        ...
        for k,(mod, sclr) in enumerate(zip(models, sclrs)):
            ...
            for i, m in enumerate(Chem.SDMolSupplier(in_fname, removeHs=False)):
                ...
                if m is not None:
                    ...
                    res=predict_mol(m, mod,sclr, i, model_type, opt_noH=opt_noH, frags=frags,
        ...
        for col in var_cols:
            df_lst[col] = (df_lst[col] <= threshold).astype(int)
...
def entry_point():
    ...
    parser.add_argument('--model_path',
                        help='Path to model ')
    parser.add_argument('-t', '--model_type', metavar='regression/classification', required=True,
                        help='')
    ...
    parser.add_argument('-m', '--multitask', metavar='True/False', required=True,
                        help='')
    parser.add_argument('--threshold', metavar='', required=False, default=0,
                        help='threshold variance for applicability domain')
    ...
    main_params(in_fname=in_fname, out_fname=out_fname, model_path=model_path, model_type=model_type, opt_noH=opt_noH, frag_fname=frag_fname,
                per_atom_fragments=per_atom_fragments, id_field_name=id_field_name, multitask=multitask, threshold=threshold, num_frag_id=num_frag_id)
