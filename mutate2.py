...
def __fragment_mol(mol, radius=3, return_ids=True, keep_stereo=False):
    """
    INPUT:
        mol - Mol
...
def mutate_mol(mol, db_cur, radius=3, min_size=1, max_size=10, min_rel_size=0, max_rel_size=1, min_inc=-2, max_inc=2, replace_cycles=False):
    ...
    def get_replacements(db_cur, env, min_atoms, max_atoms, radius):
        if radius == 3:
    ...
    for env, core, ids in f:
        ...
        if min_size <= num_heavy_atoms <= max_size \
                or min_rel_size <= hac_ratio <= max_rel_size \
            ...
            rep = get_replacements(db_cur, env, min_atoms, max_atoms, radius)
            for core_smi, core_sma in rep:
                if core_smi != core:
                    for new_mol in __frag_replace(mol, frag_sma, core_sma, ids):
