...
input_sdf_file = working_dir + "/phenylpiperizine.sdf"
...
for gen in range(num_of_gen):
    ...
    input_sdf_file = generation_dir + '/' + input_sdf_file.split('/')[-1]
    ...
    paramaters_to_predict = ['5HT1A']
    models_dir = [script_dir + '/models/5HT1A/']
    models = [['rf', 'svm', 'gbm']]
    models_type = ['reg']
    ...
    thresholds =['more9',
                 'desirability_7:0,9:(x-7)/2,100:1#5']
    ...
    process_contributions(output_sdf_file, 'fragment_contrib_norm.txt', 'worst_fragments.txt', paramaters_to_predict,
                                 models_contrib, thresholds, number_of_worst_fragments, max_frag_size)
