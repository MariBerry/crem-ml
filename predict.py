...
def get_var_names(fname, fformat):
    ...
    output = None
    if fformat == 'txt':
        output = open(fname, 'wb').readline().strip().split('\t')[1:]
    elif fformat == 'svm':
...
def save_predictions(title, fname, pred_array, pred_model_names, mol_names, ad_dict):
    # add mol_names ass the first column
    pred_array = np.column_stack((mol_names, pred_array))
    pred_model_names.insert(0, title)
...
def main_params(x_fname, input_format, out_fname, model_names, model_dir, model_type, ad, verbose, title='model'):
    ...
    save_predictions(title, out_fname, pred, pred_model_names, input_sirms_file.get_mol_names(), ad_dict)
